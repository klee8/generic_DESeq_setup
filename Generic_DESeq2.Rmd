...
SOI_list <- as.vector(samples[samples$condition == "SOI", "run"])
REF_list <- as.vector(samples[samples$condition == "REF", "run"])

# divide by the number of unique samples (some samples are duplicated over multiple lanes)
SOI_samplenum <- length(as.vector(unique(samples[samples$condition == "SOI", "sample"])))
REF_samplenum <- length(as.vector(unique(samples[samples$condition == "REF", "sample"])))

salmon_TPM <- as.data.frame (txi$abundance) %>% 
  mutate( mean_SOI_TPM = rowSums(select(., SOI_list)/SOI_samplenum),
          mean_REF_TPM = rowSums(select(., REF_list)/REF_samplenum),
          gene_id = rownames(txi$abundance)) %>%
  select(gene_id, mean_SOI_TPM, mean_REF_TPM) 
```
...
goi <- all_results %>% select(gene_id, apeglm_log2FC, apeglm_lfcSE, apeglm_1_svalue, apeglm_2_svalue, mean_REF_TPM, mean_SOI_TPM)



write.table(goi, "result_tables/SPECIES_SOI_REF.txt", quote = FALSE, row.names = FALSE, sep = "\t")
```
